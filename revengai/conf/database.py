...
class RevEngDatabase(object):
    ...
    def _cleanup_files(self):
        try:
            ...
            logger.error(
                "Error closing the connecting to local database. %s", e)
    ...
    def add_upload(self, fpath: str, sha_256_hash: str) -> None:
        try:
            with closing(self.conn.cursor()) as cursor:
                cursor.execute(
                    "INSERT OR REPLACE INTO upload(name, sha_256_hash) "
                    "VALUES (?, ?)",
                    (
                        basename(fpath),
                        sha_256_hash,
            ...
            logger.error(
                "Error adding upload into local database for hash: %s. %s",
                sha_256_hash,
                e,
            )
        finally:
    ...
    def delete_upload(self, sha_256_hash: str) -> None:
        try:
            ...
            logger.error(
                "Error deleting upload from local database for hash: %s. %s",
                sha_256_hash,
                e,
            )
        finally:
    ...
    def get_last_analysis(
        ...
        try:
            with closing(self.conn.cursor()) as cursor:
                cursor.execute(
                    "SELECT binary_id FROM analysis WHERE sha_256_hash = ? "
                    "and status = ? ORDER BY binary_id DESC LIMIT 1",
                    (
                        sha_256_hash,
                        status,
                    ),
        ...
        try:
            with closing(self.conn.cursor()) as cursor:
                cursor.execute(
                    ...
                    "(sha_256_hash, binary_id, status, creation, model_name)"
                    " VALUES (?, ?, ?, ?, ?)",
                    (
                        sha_256_hash,
                        bid,
            ...
            logger.error(
                "Error adding analysis for bid: %d, status: %s, hash: %s. %s",
                bid,
                status,
                sha_256_hash,
                e,
    ...
    def update_analysis(self, bid: int, status: str) -> None:
        try:
            with closing(self.conn.cursor()) as cursor:
                cursor.execute(
                    "UPDATE analysis SET status = ? WHERE binary_id = ?",
                    (
                        status,
                        bid,
                    ),
            ...
            logger.error(
                "Error updating analysis for bid: %d, status: %s. %s",
                bid,
                status,
                e
            )
    ...
    def delete_analysis(self, bid: int) -> None:
        try:
            with closing(self.conn.cursor()) as cursor:
                cursor.execute(
                    "DELETE FROM analysis WHERE binary_id = ?", (bid,))
        except Error as e:
            logger.error(
                "Error deleting analysis from database for bid: %d. %s", bid, e
            )
        finally:
            self.conn.commit()
    ...
    def execute_sql(self, sql: str, params: tuple[any] = None) -> None:
        try:
            ...
            logger.error("Error executing %s with params: %s. %s",
                         sql, params, e)
        finally:
            self.conn.commit()
