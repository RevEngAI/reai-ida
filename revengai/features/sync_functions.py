...
class SyncFunctionsDialog(BaseDialog):
    def __init__(self, state: RevEngState, fpath: str, data: list):
        ...
        for function in data:
            self.data.append(
                (
                    f"0x{function['function_vaddr']:08X}",
    ...
    def showEvent(self, event):
        ...
        self.ui.functionsList.setColumnWidth(0, width * 0.15)
        self.ui.functionsList.setColumnWidth(1, width * 0.75)
        self.ui.functionsList.setColumnWidth(2, width * 0.1)
    ...
    def _table_menu(self) -> None:
        ...
        if selected and isinstance(selected[2], CheckableItem):
            ...
            syncAction.triggered.connect(
                lambda: self._rename_symbol(selected[2].data))

            jumpToAction = menu.addAction("Jump to Function")
            jumpToAction.triggered.connect(
                lambda: idc.jumpto(selected[2].data["function_vaddr"])
            )
